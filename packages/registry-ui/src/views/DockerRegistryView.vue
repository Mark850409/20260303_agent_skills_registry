      ...
      <main class="browse-main">
        ...
        <template v-else>
          ...
          <div v-else class="skills-grid">
            <div v-for="(repo, i) in filteredRepositories" :key="repo.name" class="repo-card" :style="{ animationDelay: `${i * 40}ms` }">
              <div class="repo-icon">🐳</div>
              <div class="repo-info">
                <h3>{{ repo.name }}</h3>
                <p>{{ repo.description || 'Docker Repository' }}</p>
              </div>
...
const fetchCatalog = async () => {
  try {
    // 取得資料庫中倉庫資訊，如果只是字串陣列（原本的 registry API 結果），則轉成物件
    // 這裡我們預期 /api/docker/catalog 改變後應該可以回傳更多資訊，這需要稍微改一下 API，
    // 但因為時間關係，我們直接呼叫 GET /api/admin/docker-repos (如果權限允許)，
    // 不過由於 /api/docker/catalog 目前唯讀，我們可以先從 API 拿字串，然後另外去抓倉庫清單。
    
    // 理想做法：API 端點 /api/docker/catalog 同時回傳描述。
    // 在這裡我們先依賴目前的 API
    const response = await axios.get('/api/admin/docker-repos?per_page=100', {
       headers: { 'Authorization': `Bearer ${localStorage.getItem('auth_token')}` } // Assuming we can use this, otherwise we skip
    }).catch(() => null)
